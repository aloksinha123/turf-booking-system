...
export default function AdminDashboard({ apiBase, triggerAlert, token }) {
  ...
  const [activeAdminTab, setActiveAdminTab] = useState('all'); // options: 'all', 'overview', 'inventory', 'pricing', 'audit', 'telemetry'
  ...
  return (
    <div className="bg-[#070a14] min-h-screen font-sans relative text-slate-100 selection:bg-emerald-500 selection:text-slate-950">
      <ToastContainer toasts={toasts} removeToast={removeToast} />
      
      {/* 🌌 Cybernetic Glowing Header Section */}
      <div className="bg-gradient-to-b from-slate-950 via-slate-900 to-[#070a14] pt-8 pb-32 px-6 relative overflow-hidden border-b border-slate-800/60">
        {/* Ambient Neon Background Glows */}
        <div className="absolute top-0 right-1/4 w-96 h-96 bg-emerald-500/10 blur-[120px] rounded-full pointer-events-none"></div>
        <div className="absolute top-10 left-10 w-96 h-96 bg-indigo-500/10 blur-[120px] rounded-full pointer-events-none"></div>
        <div className="absolute bottom-0 right-10 w-96 h-96 bg-purple-500/10 blur-[120px] rounded-full pointer-events-none"></div>
        
        <div className="max-w-[1440px] mx-auto relative z-10">
          {/* Dashboard Header Row */}
          <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center border-b border-slate-800/80 pb-5 gap-4">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="w-3 h-7 rounded-full bg-gradient-to-b from-emerald-400 to-teal-500 inline-block shadow-[0_0_15px_rgba(16,185,129,0.6)]"></span>
                <h2 className="text-2xl md:text-3xl font-black tracking-tight text-white uppercase bg-clip-text text-transparent bg-gradient-to-r from-white via-slate-100 to-slate-400">
                  Admin Command Center
                </h2>
                <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-[10px] font-black uppercase px-3 py-1 rounded-full ml-2 shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                  Role: OWNER
                </span>
              </div>
              <p className="text-xs text-slate-400 font-semibold tracking-wide">Enterprise Multi-Turf Yield Management & Autonomous Telemetry Platform</p>
            </div>

            {/* System Telemetry & Quick Control Tools */}
            <div className="flex flex-wrap items-center gap-3">
              {/* WS Connection Status Pill */}
              <div className="flex items-center gap-2 bg-slate-900/90 border border-slate-800 px-3.5 py-2 rounded-2xl text-xs font-bold text-slate-300 backdrop-blur-xl shadow-lg">
                {wsStatus === 'connected' ? (
                  <>
                  ...
                  <>
                    ...
                    <span className="text-amber-300 font-black">🟡 Reconnecting...</span>
                  </>
                ) : (
                  <>
                    <span className="w-2.5 h-2.5 rounded-full bg-rose-400"></span>
                    <span className="text-rose-400 font-black">🔴 Polling Fallback (5s)</span>
                  </>
                )}
              </div>
              ...
              <button
                ...
                className="bg-slate-800/80 hover:bg-slate-700 border border-slate-700 text-slate-200 font-black text-xs px-4 py-2.5 rounded-2xl transition-all shadow-md cursor-pointer flex items-center gap-2 hover:scale-[1.02] active:scale-95"
              >
                📥 Export Bookings CSV
              </button>
              ...
              <button
                ...
                className={`font-black text-xs px-4 py-2.5 rounded-2xl transition-all shadow-lg cursor-pointer flex items-center gap-2 border hover:scale-[1.02] active:scale-95 ${
                  maintenanceMode.is_maintenance
                    ? 'bg-rose-600 hover:bg-rose-500 border-rose-400 text-white animate-pulse shadow-rose-600/30'
      ...
      <div className="max-w-[1440px] mx-auto px-6 -mt-24 relative z-20">
        ...
        {/* ═══════════════════════════════════════════════════════ */}
        {/* 🎛️ ADMIN CATEGORY NAVIGATION TOOLBAR                   */}
        {/* ═══════════════════════════════════════════════════════ */}
        <div className="bg-slate-900/95 border border-slate-800/90 backdrop-blur-2xl rounded-3xl p-3 mb-8 shadow-2xl sticky top-4 z-40">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            <div className="flex items-center gap-2 px-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
              <span className="text-xs font-black uppercase text-slate-300 tracking-wider">Admin Focus Mode:</span>
            </div>
            
            <div className="flex flex-wrap items-center gap-2 overflow-x-auto py-1">
              {[
                { id: 'all', label: '🌐 All-in-One View', badge: 'Full Control' },
                { id: 'overview', label: '📊 1. Overview & Analytics', badge: 'Metrics & Live Turf' },
                { id: 'inventory', label: '📅 2. Inventory & Slots', badge: 'Slots & Price Lock' },
                { id: 'pricing', label: '🏷️ 3. Dynamic Yield & Coupons', badge: 'Surge & Promos' },
                { id: 'audit', label: '📋 4. Security Audit Log', badge: 'Compliance History' },
                { id: 'telemetry', label: '⚡ 5. Concurrency Stress', badge: '1000 Worker Load' },
              ].map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveAdminTab(tab.id)}
                  className={`px-4 py-2.5 rounded-2xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer border flex items-center gap-2 ${
                    activeAdminTab === tab.id
                      ? 'bg-gradient-to-r from-emerald-500 to-teal-400 border-emerald-300 text-slate-950 shadow-[0_0_20px_rgba(16,185,129,0.4)] scale-[1.02]'
                      : 'bg-slate-950/70 border-slate-800/80 text-slate-400 hover:bg-slate-800 hover:text-white'
                  }`}
                >
                  <span>{tab.label}</span>
                </button>
        ...
        {/* ═══════════════════════════════════════════════════════ */}
        {/* SECTION 01: REAL-TIME REVENUE & TURF OVERVIEW           */}
        {/* ═══════════════════════════════════════════════════════ */}
        {(activeAdminTab === 'all' || activeAdminTab === 'overview') && (
          <div className="mb-12">
            {/* Section Heading with Role Badge */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-2 border-b border-slate-800/80">
              <div className="flex items-center gap-2.5">
                <span className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg">
                  SECTION 01
                </span>
                <h3 className="text-lg font-black text-white uppercase tracking-tight">
                  Real-Time Platform Revenue & Live Turf Matrix
                </h3>
              </div>
              <span className="text-[11px] font-semibold text-slate-400">
                💡 Role: Monitor earnings, peak booking times & instant court status
              </span>
            </div>

            {/* 💡 Section 1 Role Guide Banner */}
            <div className="bg-gradient-to-r from-emerald-500/10 via-teal-500/10 to-transparent border border-emerald-500/30 rounded-2xl p-4 mb-6 flex items-start gap-3.5 backdrop-blur-xl">
              <div className="w-9 h-9 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-lg flex-shrink-0 border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                ℹ️
              </div>
              <div>
                <h4 className="text-xs font-black uppercase text-emerald-300 tracking-wider">How to use Section 1 (Analytics & Overview):</h4>
                <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                  Use this view to inspect live business performance. The <strong>KPI cards</strong> show overall platform gross revenue, court occupancy percentages, active bookings, and net 5% commission earnings. The <strong>Hourly Chart</strong> highlights peak usage hours so you know when demand is highest. The <strong>Live Turf Matrix</strong> gives real-time operational status (e.g. Match in Progress, Open Slots, or Maintenance Lock).
                </p>
              </div>
            </div>

            {/* MULTI-RANGE REVENUE & BOOKING FILTER TOOLBAR */}
            <div className="bg-slate-900/90 border border-slate-800/90 backdrop-blur-xl rounded-3xl p-4 mb-6 shadow-2xl flex flex-wrap items-center justify-between gap-4 text-white">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs font-black uppercase text-slate-400 mr-2 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400"></span> 📅 Revenue Range:
                </span>
                {['today', 'weekly', 'monthly', 'yearly', 'custom'].map((r) => (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer border ${
                      analyticsRange === r
                        ? 'bg-gradient-to-r from-emerald-500 to-teal-400 border-emerald-300 text-slate-950 shadow-[0_0_15px_rgba(16,185,129,0.4)]'
              ...
              <div className="flex items-center gap-3">
                {analyticsRange === 'custom' && (
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-1.5 text-xs font-bold text-white focus:outline-none focus:border-emerald-500"
                    />
                ...
                <select
                  ...
                  className="bg-slate-800/90 border border-slate-700/80 rounded-xl px-3.5 py-1.5 text-xs font-bold text-white focus:outline-none cursor-pointer hover:border-slate-600 transition-colors"
                >
            ...
            {v2Analytics?.ai_insights?.length > 0 && (
              <div className="bg-gradient-to-r from-indigo-950/90 via-slate-900 to-purple-950/90 border border-indigo-500/40 rounded-3xl p-5 mb-6 text-white shadow-[0_10px_30px_rgba(99,102,241,0.15)] backdrop-blur-xl flex items-start gap-4">
                <div className="w-11 h-11 bg-indigo-500/20 text-indigo-300 rounded-2xl flex items-center justify-center font-black text-2xl flex-shrink-0 border border-indigo-500/30 shadow-[0_0_15px_rgba(99,102,241,0.3)]">
                  🧠
                </div>
                <div className="flex-grow">
                  <span className="text-[10px] font-black uppercase text-indigo-300 tracking-widest bg-indigo-500/20 px-2.5 py-1 rounded-full border border-indigo-500/30">
                    AI Automated Occupancy & Revenue Insights
                  </span>
                  <div className="mt-2.5 space-y-1.5 text-xs font-bold text-slate-200">
                    {v2Analytics.ai_insights.map((insight, idx) => (
                      <p key={idx} className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-400"></span> {insight}
                      </p>
                    ))}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
              ...
              <div className="bg-slate-900/90 border border-slate-800/90 border-t-2 border-t-emerald-400 shadow-2xl rounded-3xl p-6 relative overflow-hidden group hover:border-slate-700 hover:-translate-y-1 transition-all duration-300">
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="w-9 h-9 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                    <svg className="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  </div>
                </div>
                <h3 className="text-3xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-300 to-cyan-300 break-all">
                  ₹{analytics.total_revenue?.toLocaleString('en-IN') || '0'}
                </h3>
                <div className="mt-3 inline-flex items-center gap-1.5 bg-emerald-500/10 text-emerald-400 px-2.5 py-1 rounded-lg text-[10px] font-black border border-emerald-500/20 uppercase tracking-wider whitespace-nowrap">
              ...
              <div className="bg-slate-900/90 border border-slate-800/90 border-t-2 border-t-cyan-400 shadow-2xl rounded-3xl p-6 relative overflow-hidden group hover:border-slate-700 hover:-translate-y-1 transition-all duration-300">
                <div className="absolute top-0 right-0 w-28 h-28 bg-gradient-to-br from-cyan-500/10 to-transparent rounded-bl-full pointer-events-none"></div>
                <div className="flex justify-between items-start mb-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Occupancy Rate</span>
                  <div className="w-9 h-9 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center shadow-[0_0_10px_rgba(6,182,212,0.2)]">
                ...
                <h3 className="text-3xl font-black tracking-tight text-white break-all">
                  {occupancyRate}%
                </h3>
                <div className="mt-3 w-full h-2 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-cyan-400 via-teal-400 to-emerald-400 rounded-full transition-all duration-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]"
                    style={{ width: `${Math.min(occupancyRate, 100)}%` }}
                  ></div>
                </div>
                <p className="text-[10px] text-slate-400 font-bold mt-2">{bookedSlots} / {totalSlots} slots filled</p>
              </div>

              {/* KPI 3: Active Bookings Today */}
              <div className="bg-slate-900/90 border border-slate-800/90 border-t-2 border-t-purple-400 shadow-2xl rounded-3xl p-6 relative overflow-hidden group hover:border-slate-700 hover:-translate-y-1 transition-all duration-300">
                <div className="absolute top-0 right-0 w-28 h-28 bg-gradient-to-br from-purple-500/10 to-transparent rounded-bl-full pointer-events-none"></div>
                <div className="flex justify-between items-start mb-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Bookings</span>
                  <div className="w-9 h-9 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center shadow-[0_0_10px_rgba(168,85,247,0.2)]">
                ...
                <div className="mt-3 inline-flex items-center gap-1.5 bg-purple-500/10 text-purple-300 px-2.5 py-1 rounded-lg text-[10px] font-black border border-purple-500/20 uppercase tracking-wider whitespace-nowrap">
                  <span className="w-2 h-2 rounded-full bg-purple-400 flex-shrink-0 animate-pulse"></span> Across all turfs
                </div>
              </div>

              {/* KPI 4: Platform Commission (5%) */}
              <div className="bg-slate-900/90 border border-slate-800/90 border-t-2 border-t-amber-400 shadow-2xl rounded-3xl p-6 relative overflow-hidden group hover:border-slate-700 hover:-translate-y-1 transition-all duration-300">
                <div className="absolute top-0 right-0 w-28 h-28 bg-gradient-to-br from-amber-500/10 to-transparent rounded-bl-full pointer-events-none"></div>
                <div className="flex justify-between items-start mb-3">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Platform Commission (5%)</span>
                  <div className="w-9 h-9 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center shadow-[0_0_10px_rgba(245,158,11,0.2)]">
                ...
                <h3 className="text-3xl font-black tracking-tight text-amber-400 break-all">
                  ₹{parseInt(commission).toLocaleString('en-IN')}
                </h3>
                <div className="mt-3 inline-flex items-center gap-1.5 bg-amber-500/10 text-amber-300 px-2.5 py-1 rounded-lg text-[10px] font-black border border-amber-500/20 uppercase tracking-wider whitespace-nowrap">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <div className="lg:col-span-2 bg-slate-900/90 border border-slate-800/90 shadow-2xl rounded-3xl p-6 relative overflow-hidden backdrop-blur-xl">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2">
                      ...
                      Hourly Booking Volume & Revenue Curve
                    </h3>
                    <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Weekly aggregate view</p>
                  </div>
                  <div className="flex gap-4 text-[10px] font-black uppercase tracking-wider">
                    <span className="flex items-center gap-1.5 text-cyan-400"><span className="w-2 h-2 rounded-full bg-cyan-400 inline-block shadow-[0_0_6px_rgba(6,182,212,0.8)]"></span> Bookings</span>
                    <span className="flex items-center gap-1.5 text-purple-400"><span className="w-2 h-2 rounded-full bg-purple-400 inline-block shadow-[0_0_6px_rgba(168,85,247,0.8)]"></span> Revenue</span>
                  </div>
                </div>
                ...
                <div className="relative h-52">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 flex justify-between px-1">
                    {chartHours.map(h => (
                      <span key={h} className="text-[9px] text-slate-500 font-bold">{h}:00</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="mt-4 flex gap-4 border-t border-slate-800/80 pt-4">
                  <div className="bg-slate-950/60 border border-slate-800 rounded-2xl px-4 py-2.5 flex-1">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Peak Hour</span>
                    <span className="text-sm font-black text-cyan-400">{peakHourStr}</span>
                  </div>
                  <div className="bg-slate-950/60 border border-slate-800 rounded-2xl px-4 py-2.5 flex-1">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Avg Volume</span>
                    <span className="text-sm font-black text-purple-400">{avgVolume} bookings/hr</span>
                  </div>
                  <div className="bg-slate-950/60 border border-slate-800 rounded-2xl px-4 py-2.5 flex-1">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Revenue Peak</span>
                    <span className="text-sm font-black text-emerald-400">{revenuePeak}</span>
                  </div>
                </div>
              </div>

              {/* RIGHT: Live Turf Status Matrix (1/3 width) */}
              <div className="bg-slate-900/90 border border-slate-800/90 shadow-2xl rounded-3xl p-6 backdrop-blur-xl">
                <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2 mb-1">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse shadow-[0_0_8px_rgba(52,211,153,0.8)] inline-block"></span>
                  Live Turf Status Matrix
                </h3>
                <p className="text-[10px] text-slate-400 font-semibold mb-5">Instant toggle & monitoring</p>
                ...
                <div className="space-y-4">
                  ...
                  {[
                    ...
                    return (
                      <div key={turf.id} className={`${c.bg} border ${c.border} rounded-2xl p-4 transition-all`}>
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h4 className="text-xs font-black text-white uppercase tracking-wider">{turf.name} ({turf.sport})</h4>
                            <p className="text-[9px] text-slate-400 font-bold">{turf.venue}</p>
                          </div>
                        ...
                        <div className="flex gap-2 mt-3">
                          <div className="bg-slate-950/60 border border-slate-800 rounded-xl px-2.5 py-1.5 flex-1 text-center">
                            <span className="text-[8px] text-slate-400 font-black uppercase block">Booked</span>
                            <span className="text-xs font-black text-white">{slots.filter(s => s.turf_id === turf.id && s.is_booked).length}</span>
                          </div>
                          <div className="bg-slate-950/60 border border-slate-800 rounded-xl px-2.5 py-1.5 flex-1 text-center">
                            <span className="text-[8px] text-slate-400 font-black uppercase block">Open</span>
                            <span className="text-xs font-black text-emerald-400">{slots.filter(s => s.turf_id === turf.id && !s.is_booked && !s.is_locked).length}</span>
                          </div>
                          <div className="bg-slate-950/60 border border-slate-800 rounded-xl px-2.5 py-1.5 flex-1 text-center">
                            <span className="text-[8px] text-slate-400 font-black uppercase block">Locked</span>
                            <span className="text-xs font-black text-amber-400">{slots.filter(s => s.turf_id === turf.id && s.is_locked).length}</span>
                          </div>
                        </div>
                      </div>
                    );
        ...
        {/* ═══════════════════════════════════════════════════════ */}
        {/* SECTION 02: INVENTORY & BUSINESS CONTROLS              */}
        {/* ═══════════════════════════════════════════════════════ */}
        {(activeAdminTab === 'all' || activeAdminTab === 'inventory') && (
          <div className="mb-12">
            {/* Section Heading with Role Badge */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-2 border-b border-slate-800/80">
              <div className="flex items-center gap-2.5">
                <span className="bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg">
                  SECTION 02
                </span>
                <h3 className="text-lg font-black text-white uppercase tracking-tight">
                  Inventory Controls & Slot Matrix
                </h3>
              </div>
              <span className="text-[11px] font-semibold text-slate-400">
                💡 Role: Slot generation, price overrides & maintenance freezing
              </span>
            </div>

            {/* 💡 Section 2 Role Guide Banner */}
            <div className="bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-transparent border border-cyan-500/30 rounded-2xl p-4 mb-6 flex items-start gap-3.5 backdrop-blur-xl">
              <div className="w-9 h-9 rounded-xl bg-cyan-500/20 text-cyan-400 flex items-center justify-center font-bold text-lg flex-shrink-0 border border-cyan-500/30 shadow-[0_0_10px_rgba(6,182,212,0.2)]">
                ℹ️
              </div>
              <div>
                <h4 className="text-xs font-black uppercase text-cyan-300 tracking-wider">How to use Section 2 (Inventory & Slots):</h4>
                <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                  Click <strong>"⚡ Generate Today"</strong> to create 10 AM – 10 PM hourly slots for the chosen date. Select checkboxes next to slots to perform <strong>Bulk Edit Price</strong> or <strong>Freeze / Unfreeze</strong>. Use the <strong>Surge Multiplier slider</strong> to apply instant global price surges across all slots. Hover over any row to manually override individual slot prices.
                </p>
              </div>
            </div>

            <div className="bg-slate-900/90 border border-slate-800/90 shadow-2xl rounded-3xl overflow-hidden backdrop-blur-xl">

              {/* Section Header */}
              <div className="p-6 border-b border-slate-800/80 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                <div>
                  <h3 className="text-base font-black text-white uppercase tracking-wider flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.8)] inline-block"></span>
                    Turf Inventory & Slot Manager
                  </h3>
                  <p className="text-xs text-slate-400 font-semibold mt-1">{slots.length} slots loaded for {selectedDate} · Multiplier: {multiplier}x</p>
                </div>
              </div>
              ...
              <div className="p-4 border-b border-slate-800/80 bg-slate-950/60">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-2xl px-3.5 py-2 shadow-inner">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider whitespace-nowrap">📅 Date:</span>
                      <input
                        type="date"
                        value={selectedDate}
                        onChange={(e) => setSelectedDate(e.target.value)}
                        className="bg-transparent border-none text-white text-[11px] font-bold outline-none cursor-pointer w-32"
                    ...
                    <button
                      ...
                      className="bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-black text-[9px] uppercase tracking-wider py-2 px-3.5 rounded-xl transition-all active:scale-95 disabled:opacity-50 cursor-pointer whitespace-nowrap shadow-md"
                    >
                      {isGenerating ? '...' : '⚡ Generate Today (10AM–10PM)'}
                    </button>
                    <button
                      onClick={() => setBulkGenModal({ ...bulkGenModal, show: true })}
                      className="bg-indigo-600 hover:bg-indigo-500 text-white font-black text-[9px] uppercase tracking-wider py-2 px-3.5 rounded-xl transition-all active:scale-95 cursor-pointer whitespace-nowrap shadow-md border border-indigo-500/40"
                    >
                  ...
                  <div className="flex items-center gap-2 bg-slate-950 border border-slate-800 text-white rounded-2xl px-3.5 py-2 shadow-inner">
                    ...
                    <button
                      ...
                      className="bg-slate-800 hover:bg-slate-700 text-white font-black text-[9px] uppercase tracking-wider py-1.5 px-3 rounded-lg transition-all disabled:opacity-40 cursor-pointer border border-slate-700"
                    >
                      ✏️ Edit Price
                    </button>
                    <button
                      ...
                      disabled={selectedSlotIDs.length === 0}
                      className="bg-rose-500/20 text-rose-300 hover:bg-rose-500/40 border border-rose-500/30 font-black text-[9px] uppercase tracking-wider py-1.5 px-3 rounded-lg transition-all disabled:opacity-40 cursor-pointer"
                    >
                      🔒 Freeze
                    </button>
                    <button
                      ...
                      disabled={selectedSlotIDs.length === 0}
                      className="bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/40 border border-emerald-500/30 font-black text-[9px] uppercase tracking-wider py-1.5 px-3 rounded-lg transition-all disabled:opacity-40 cursor-pointer"
                    >
                      🟢 Unfreeze
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-2xl px-3.5 py-2 shadow-inner">
                    <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider whitespace-nowrap">🌐 Surge:</span>
                    <input
                      type="number"
                      step="0.1"
                      min="0.5"
                      ...
                      className="bg-slate-950 border border-slate-800 text-white text-sm font-black rounded-xl p-1.5 outline-none text-center w-16 focus:border-purple-500/50 transition-colors"
                    />
                    <button
                      onClick={handleApplyMultiplier}
                      disabled={isApplying}
                      className="bg-purple-600 hover:bg-purple-500 text-white font-black text-[9px] uppercase tracking-wider py-2 px-3.5 rounded-xl transition-all active:scale-95 disabled:opacity-50 cursor-pointer whitespace-nowrap border border-purple-500/40 shadow-md"
                    >
                  ...
                  <div className="hidden lg:flex items-center gap-2 flex-1 min-w-[140px]">
                    <span className="text-[8px] font-black text-cyan-400">0.5x</span>
                    <div className="flex-1 h-1.5 bg-slate-800 rounded-full relative overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-300 shadow-[0_0_8px_rgba(16,185,129,0.5)]"
                        style={{
                    ...
                    <span className="text-[8px] font-black text-rose-400">3.0x</span>
                  </div>
                  ...
                  <button
                    ...
                    disabled={isSeeding}
                    className="ml-auto bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-emerald-500/40 text-slate-300 hover:text-emerald-400 text-[9px] font-black uppercase tracking-wider py-2.5 px-3.5 rounded-2xl transition-all active:scale-95 disabled:opacity-50 cursor-pointer whitespace-nowrap shadow-md"
                  >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead>
                    <tr className="text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-800 bg-slate-950/80 sticky top-0 z-10">
                      ...
                      <th scope="col" className="px-4 py-3.5">Slot ID</th>
                      <th scope="col" className="px-4 py-3.5">Timing</th>
                      <th scope="col" className="px-4 py-3.5">Turf Name</th>
                      <th scope="col" className="px-4 py-3.5">Base Price</th>
                      <th scope="col" className="px-4 py-3.5">Dynamic Multiplier</th>
                  ...
                  <tbody>
                    {slots.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="px-4 py-12 text-center">
                          <div className="flex flex-col items-center gap-2 text-slate-500">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">No slots generated</span>
                            <span className="text-[9px] font-semibold text-slate-500">Click "⚡ Generate Today" above to create inventory</span>
                          </div>
                        </td>
                      </tr>
                    ) : (
                      slots.map((slot) => {
                        ...
                        return (
                          <tr key={slot.id} className="border-b border-slate-800/60 hover:bg-slate-800/40 transition-colors group">
                            {/* Selection Checkbox */}
                            ...
                            <td className="px-4 py-3.5 font-black text-white text-xs whitespace-nowrap">
                              <span className="text-slate-500">#</span>{String(slot.id).padStart(3, '0')}
                            </td>

                            {/* Timing */}
                            <td className="px-4 py-3.5">
                              <span className="bg-slate-950 text-slate-300 py-1 px-2.5 rounded-lg font-mono font-bold text-[10px] border border-slate-800">
                            ...
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-2">
                                {multiplier !== 1.0 ? (
                                  <>
                                    <span className="text-slate-500 line-through text-[10px] font-bold">₹{Math.round(slot.base_price)}</span>
                                    <span className="text-emerald-400 font-black text-xs">₹{Math.round(dynamicPrice)}</span>
                                  </>
                                ) : (
                                  <span className="text-emerald-400 font-black text-xs">₹{Math.round(slot.base_price)}</span>
                                )}
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3.5">
                              <span className={`${badgeColors[badge.color] || badgeColors.slate} border font-black px-2.5 py-0.5 rounded-lg text-[9px] uppercase tracking-wider inline-flex items-center gap-1`}>
                                {badge.label}
                              </span>
                            </td>
        ...
        {/* ═══════════════════════════════════════════════════════ */}
        {/* SECTION 03: DYNAMIC YIELD RULES & COUPON ENGINE         */}
        {/* ═══════════════════════════════════════════════════════ */}
        {(activeAdminTab === 'all' || activeAdminTab === 'pricing') && (
          <div className="mb-12">
            {/* Section Heading with Role Badge */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-2 border-b border-slate-800/80">
              <div className="flex items-center gap-2.5">
                <span className="bg-purple-500/20 text-purple-400 border border-purple-500/30 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg">
                  SECTION 03
                </span>
                <h3 className="text-lg font-black text-white uppercase tracking-tight">
                  Dynamic Yield Rules Engine & Promotional Coupons
                </h3>
              </div>
              <span className="text-[11px] font-semibold text-slate-400">
                💡 Role: Automate demand surge pricing & issue discount promo codes
              </span>
            </div>

            {/* 💡 Section 3 Role Guide Banner */}
            <div className="bg-gradient-to-r from-purple-500/10 via-indigo-500/10 to-transparent border border-purple-500/30 rounded-2xl p-4 mb-6 flex items-start gap-3.5 backdrop-blur-xl">
              <div className="w-9 h-9 rounded-xl bg-purple-500/20 text-purple-400 flex items-center justify-center font-bold text-lg flex-shrink-0 border border-purple-500/30 shadow-[0_0_10px_rgba(168,85,247,0.2)]">
                ℹ️
              </div>
              <div>
                <h4 className="text-xs font-black uppercase text-purple-300 tracking-wider">How to use Section 3 (Yield Engine & Promo Codes):</h4>
                <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                  The <strong>Yield Pricing Rules Engine</strong> applies automatic price multipliers (e.g. 1.25x for Diwali or weekend prime slots) based on rule priorities. Higher priority numbers override lower priorities. The <strong>Promotional Coupon Engine</strong> allows you to issue percentage discount codes (e.g. <code>TURF20</code>) and control whether coupons can be used during surge pricing events.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-slate-900/90 border border-slate-800/90 backdrop-blur-xl rounded-3xl p-6 shadow-2xl">
                ...
                {/* Create Rule Form */}
                <form onSubmit={handleCreateRule} className="grid grid-cols-2 gap-3 mb-6 bg-slate-950/60 p-4 rounded-2xl border border-slate-800">
                  <div className="col-span-2 sm:col-span-1">
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3.5 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold text-white focus:outline-none focus:border-indigo-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3.5 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold text-white focus:outline-none focus:border-indigo-500 cursor-pointer"
                    >
                  ...
                  <div className="col-span-2">
                    <button
                      ...
                      className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase text-xs tracking-wider py-3 rounded-xl cursor-pointer shadow-md border border-indigo-500/40 hover:scale-[1.01] transition-all"
                    >
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {pricingRules.length === 0 ? (
                    ...
                    pricingRules.map((rule) => (
                      <div key={rule.id} className="flex justify-between items-center bg-slate-950/80 border border-slate-800/80 p-3 rounded-xl text-xs">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-white">{rule.name}</span>
                            <span className="text-[9px] font-black bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 px-2 py-0.5 rounded uppercase">
              ...
              <div className="bg-slate-900/90 border border-slate-800/90 backdrop-blur-xl rounded-3xl p-6 shadow-2xl">
                ...
                {/* Create Coupon Form */}
                <form onSubmit={handleCreateCoupon} className="grid grid-cols-2 gap-3 mb-6 bg-slate-950/60 p-4 rounded-2xl border border-slate-800">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3.5 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold text-white uppercase focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full px-3.5 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold text-white focus:outline-none focus:border-emerald-500"
                    />
                  </div>

                  <div className="col-span-2 flex items-center justify-between bg-slate-900 p-3 rounded-xl border border-slate-800">
                  ...
                  <div className="col-span-2">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-400 hover:to-teal-300 text-slate-950 font-black uppercase text-xs tracking-wider py-3 rounded-xl cursor-pointer shadow-md hover:scale-[1.01] transition-all"
                    >
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {coupons.length === 0 ? (
                    ...
                    coupons.map((c) => (
                      <div key={c.id} className="flex justify-between items-center bg-slate-950/80 border border-slate-800/80 p-3 rounded-xl text-xs">
                        <div>
        ...
        {/* ═══════════════════════════════════════════════════════ */}
        {/* SECTION 04: SECURITY AUDIT & ADMIN LOGS                 */}
        {/* ═══════════════════════════════════════════════════════ */}
        {(activeAdminTab === 'all' || activeAdminTab === 'audit') && (
          <div className="mb-12">
            {/* Section Heading with Role Badge */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-2 border-b border-slate-800/80">
              <div className="flex items-center gap-2.5">
                <span className="bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg">
                  SECTION 04
                </span>
                <h3 className="text-lg font-black text-white uppercase tracking-tight">
                  Security Audit & Admin Activity Log
                </h3>
              </div>
              <span className="text-[11px] font-semibold text-slate-400">
                💡 Role: Compliance audit, IP tracking & operational history
              </span>
            </div>

            {/* 💡 Section 4 Role Guide Banner */}
            <div className="bg-gradient-to-r from-amber-500/10 via-orange-500/10 to-transparent border border-amber-500/30 rounded-2xl p-4 mb-6 flex items-start gap-3.5 backdrop-blur-xl">
              <div className="w-9 h-9 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold text-lg flex-shrink-0 border border-amber-500/30 shadow-[0_0_10px_rgba(245,158,11,0.2)]">
                ℹ️
              </div>
              <div>
                <h4 className="text-xs font-black uppercase text-amber-300 tracking-wider">How to use Section 4 (Security Audit Trail):</h4>
                <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                  Every critical administrative action (such as modifying slot prices, freezing inventory, toggling maintenance mode, or creating dynamic pricing rules) is automatically recorded here in real-time. Review timestamps, admin usernames, role levels, target resources, action details, and IP addresses for full security compliance.
                </p>
              </div>
            </div>

            <div className="bg-slate-900/90 border border-slate-800/90 backdrop-blur-xl rounded-3xl p-6 shadow-2xl text-white">
              <div className="flex justify-between items-center mb-4 border-b border-slate-800/80 pb-3">
                <div>
                  <h3 className="text-lg font-black uppercase tracking-tight flex items-center gap-2">
                    <span>📋</span> Security Audit Log History
                  </h3>
                  <p className="text-xs text-slate-400 font-medium mt-0.5">Real-time audit log of all admin operations</p>
                </div>
                <span className="text-xs font-black text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-3.5 py-1.5 rounded-full uppercase">
              ...
              <div className="overflow-x-auto max-h-60 overflow-y-auto">
                <table className="w-full text-left text-xs">
                  <thead className="text-[10px] font-black uppercase text-slate-400 border-b border-slate-800 bg-slate-950/90 sticky top-0">
                    <tr>
                    ...
                    {activityLogs.length === 0 ? (
                      ...
                      activityLogs.map((log) => (
                        <tr key={log.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="py-2.5 px-3.5 whitespace-nowrap text-slate-400">{new Date(log.created_at).toLocaleString()}</td>
                          <td className="py-2.5 px-3.5 font-bold text-white">{log.admin_name}</td>
                          ...
                          <td className="py-2.5 px-3.5 font-bold text-amber-400">{log.action}</td>
                          <td className="py-2.5 px-3.5 text-cyan-300">{log.target_resource}</td>
                          <td className="py-2.5 px-3.5 text-slate-400">{log.details}</td>
                          <td className="py-2.5 px-3.5 text-slate-500 font-mono text-[10px]">{log.ip_address}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
        ...
        {/* ═══════════════════════════════════════════════════════ */}
        {/* SECTION 05: CONCURRENCY STRESS ENGINE & TELEMETRY       */}
        {/* ═══════════════════════════════════════════════════════ */}
        {(activeAdminTab === 'all' || activeAdminTab === 'telemetry') && (
          <div className="mb-12">
            {/* Section Heading with Role Badge */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-2 border-b border-slate-800/80">
              <div className="flex items-center gap-2.5">
                <span className="bg-rose-500/20 text-rose-400 border border-rose-500/30 text-[10px] font-black uppercase px-2.5 py-1 rounded-lg">
                  SECTION 05
                </span>
                <h3 className="text-lg font-black text-white uppercase tracking-tight">
                  Production Concurrency Stress Engine & Telemetry
                </h3>
              </div>
              <span className="text-[11px] font-semibold text-slate-400">
                💡 Role: High-load stress benchmarking & database row locking verification
              </span>
            </div>

            {/* 💡 Section 5 Role Guide Banner */}
            <div className="bg-gradient-to-r from-rose-500/10 via-pink-500/10 to-transparent border border-rose-500/30 rounded-2xl p-4 mb-6 flex items-start gap-3.5 backdrop-blur-xl">
              <div className="w-9 h-9 rounded-xl bg-rose-500/20 text-rose-400 flex items-center justify-center font-bold text-lg flex-shrink-0 border border-rose-500/30 shadow-[0_0_10px_rgba(244,63,94,0.2)]">
                ℹ️
              </div>
              <div>
                <h4 className="text-xs font-black uppercase text-rose-300 tracking-wider">How to use Section 5 (Stress Engine & Load Benchmarking):</h4>
                <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                  Use this tool to run live stress tests with <strong>10 to 1,000 parallel workers</strong> attempting to book the exact same court slot simultaneously. Verify that PostgreSQL row-level locks prevent double-bookings, monitor P50/P95/P99 latency distribution curves, and download detailed CSV benchmark reports.
                </p>
              </div>
            </div>
              ...
              <form onSubmit={handleBulkPriceSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-2xl text-sm font-bold text-white focus:outline-none focus:border-cyan-500"
                  />
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 bg-slate-800 hover:bg-slate-700 font-black text-xs uppercase py-3 rounded-2xl transition-all cursor-pointer text-slate-400"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-black text-xs uppercase py-3 rounded-2xl transition-all cursor-pointer shadow-md"
                  >
                    Apply Bulk Price
                  </button>
                </div>
              ...
              <form onSubmit={handleBulkGenSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs font-bold text-white focus:outline-none cursor-pointer"
                  >
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs uppercase py-3 rounded-2xl transition-all cursor-pointer shadow-md"
                  >
                    Generate Slots
                  </button>
                </div>
